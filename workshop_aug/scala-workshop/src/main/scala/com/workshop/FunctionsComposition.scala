...
object FunctionsComposition {
  ...
  def addShippingThenCalculatePrice(price: Double): Double = (applyTax compose applyDiscount compose applyShipping)(price)
//  def addShippingThenCalculatePrice(price: Double): Double = (applyShipping compose applyDiscount compose applyTax)(price)
}
