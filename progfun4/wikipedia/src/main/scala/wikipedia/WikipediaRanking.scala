...
object WikipediaRanking {
  ...
  val sc: SparkContext = new SparkContext("local[*]", "WikipediaRanking", conf)
  ...
  def rankLangsUsingIndex(index: RDD[(String, Iterable[WikipediaArticle])]): List[(String, Int)] =
    index.mapValues(_.count(_ => true)).collect().sortWith(_._2 > _._2).toList
...
//    val pairs = rdd.flatMap(article => langs.filter(article.mentionsLanguage(_)).map((_, 1)))
    pairs.reduceByKey(_ + _).collect().sortWith(_._2 > _._2).toList
  }
